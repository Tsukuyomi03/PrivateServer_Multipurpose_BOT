module.exports = client =>{
    ...
    const updateMembers = guild =>{
        ...
        channel4.setName(`ONLINE : ${guild.members.cache.filter(m =>m.presence.status === 'online' && === 'idle' && === 'dnd').size}`)
        const channel5 = guild.channels.cache.get(channelID5)
        channel5.setName(`OFFLINE : ${guild.members.cache.filter(m =>m.presence.status !== 'online' && !== 'idle' && !== 'dnd').size}`)
